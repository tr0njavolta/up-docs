...
- An externally-managed PostgreSQL with its own HA story. Hub itself goes
  ...
  tier. See [the database overview](./databases/overview.md).
- Sized resource requests for `hub-api`. Pick a tier in [sizing](./sizing.md)
...
- [Autoscaling](./autoscaling.md). Let `hub-api` grow and shrink with load instead
