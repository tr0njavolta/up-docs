...
OIDC overview](/hub/oidc-configuration) for the provider-specific
group-claim setup.
...
- Revisit the OIDC group-claim setup if group identities don't surface as
  expected: [OIDC overview](/hub/oidc-configuration).
- Return to the [production hardening overview](/hub/production-overview).
