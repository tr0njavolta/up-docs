...
description: FAQ, debugging, and glossary for Hub.
---
...
- Is Hub read-only? Yes. It doesn't manage control plane lifecycle.
...
- **Hub.** The product for seeing, searching, and querying every control
    plane through one API and the Console.
- **Console.** The web UI for Hub. Replaces the retired Upbound Console.
- **`hub-api`.** The Hub API server. A runtime component, not a user-facing
    name.
- **`hub-connector`.** The per-control-plane agent that pushes state to `hub-api`.
- **Control plane connection.** A registered control plane that Hub watches.
