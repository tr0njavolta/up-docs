...
A Composition *composes* individual resources together into a larger, reusable,
solution.
...
For example, to install [Function Patch and Transform][function-patch-and-transform],
...
resource installed by a [provider][providers], or
defined by an XRD. This includes all
[MRs][managed-resources] and
[XRs][xrs]. It also has access to some types of
...
:::note
The [RBAC manager][rbac-manager] automatically
...
<details>
<summary>The xr.yaml, composition.yaml and function.yaml files used in the example</summary>
...
:::tip
A *composed* resource is a resource created by a composite resource. Composed
resources can be any kind of Kubernetes resource.
:::

### Observed state

When you create a composite resource like this one, Crossplane *observes* it and
...
A pipeline of functions *accumulates* desired state. This means that each
...
A function can *intentionally* choose not to copy parts of the desired state.
...
terminology, the desired state is a *fully specified intent*.
:::
