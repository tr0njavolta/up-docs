...
1. Verify that `etcd` is running with three replicas.
   ...
   Each control plane namespace (`mxp-<uuid>-system`) should have a
   `vcluster-etcd` PDB with `ALLOWED DISRUPTIONS: 1`. If no PDBs appear, your
   Space is running single-replica `etcd`. Enable HA before continuing.
...
2. Verify all `etcd` pods are healthy.
   ...
   All pods should show `Running` with a stable restart count. Do not proceed if
   any pod is in `Pending`, `CrashLoopBackOff`, or `Error`.

3. Suspend the `etcd` defrag CronJob.

   Each control plane runs a scheduled `etcdctl defrag --cluster` job. Suspend
   it during the rotation to avoid adding load:
...
Rotate one node at a time. Kubernetes enforces the PDB to prevent more than one
`etcd` replica from going unavailable simultaneously.
...
2. Drain the node.
   ...
   The drain may take several minutes. If the `etcd` pod eviction is blocked by
   the PDB, Kubernetes retries until it's safe to proceed. This is expected —
   don't force the drain.
...
5. Verify `etcd` pods reschedule and reach `Running` state before moving to the
   next node.
...
3. Check for control planes stuck in a non-`Ready` state.
   ...
   After a node rotation, `spaces-controller` and `mxp-controller` may fail to
   reconnect to a control plane's API server and require a manual restart. If
   control planes remain in a non-`Ready` state 10 minutes after the rotation
   completes, restart the controllers:
...
The drain is waiting for the PDB — draining would violate `etcd` quorum. Check
which pods aren't yet ready:
...
If a control plane remains unhealthy and restarting controllers doesn't resolve
it, collect diagnostics before restarting any pods. The reconnection failure is
transient and logs disappear after a restart:
